<template>
  <div class="w-full lg:w-80 xl:w-96 bg-slate-50 border-r border-slate-200 overflow-y-auto transition-all duration-300 h-full">
    <div class="p-5">
      <div class="mb-6 space-y-4">
        ...
        <div class="relative w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
          <div class="absolute inset-0 bg-gradient-to-r from-maroon-500 to-maroon-600 rounded-full transition-all duration-500"
               :style="{ width: `${(completedLectureCount/totalLectures) * 100}%` }"></div>
        </div>
      </div>
      
      <div class="space-y-3">
        <div v-for="(week, weekIndex) in weeks" :key="weekIndex" 
             class="group bg-white rounded-xl border border-slate-200 hover:border-maroon-200
          ...
          <div class="flex items-center justify-between p-4 cursor-pointer"
            ...
            <div class="space-y-1">
              ...
              <p class="text-sm text-slate-500">{{ week.lectures.length }} modules • {{ week.duration }}</p>
            </div>
            <div class="flex items-center space-x-2">
              ...
              <span class="material-symbols-outlined text-maroon-400 transition-transform duration-300 transform group-hover:text-maroon-600"
                    :class="[week.isExpanded ? 'rotate-180' : '']">expand_more</span>
            </div>
          </div>
          ...
          <div v-show="week.isExpanded" class="border-t border-slate-100 pt-2">
            <div v-for="(lecture, lectureIndex) in week.lectures" 
                 ...
                 class="flex items-start p-3 mx-2 mb-2 rounded-lg cursor-pointer transition-all duration-300
                        hover:bg-maroon-50/50 border border-transparent hover:border-maroon-200"
                 :class="{'bg-maroon-50 border-maroon-200': lecture.id === selectedLecture?.id}">
              <div class="flex-shrink-0 mt-1.5">
                <span class="material-symbols-outlined text-sm transform transition-all"
                   :class="[
                     lecture.completed ? 'text-emerald-500 scale-110' : 
                     lecture.id === selectedLecture?.id ? getActivityIcon(lecture.type).color + ' scale-110' :
                     getActivityIcon(lecture.type).baseColor + ' hover:' + getActivityIcon(lecture.type).color
                   ]">{{ lecture.completed ? 'check_circle' : getActivityIcon(lecture.type).materialIcon }}</span>
              </div>
              <div class="ml-3 flex-grow">
                ...
                <div class="flex items-center mt-1.5 space-x-3">
                  ...
                  <div v-if="lecture.type !== 'video'" 
                    ...
                    <span class="material-symbols-outlined text-sm">{{ getActivityBadgeIcon(lecture.type) }}</span>
                    <span>{{ getActivityLabel(lecture.type) }}</span>
                  </div>
                  <div v-if="lecture.materials?.length" class="flex items-center space-x-1 text-slate-500 text-xs">
