...
## THIS LIFECYCLE LOOP NEEDS TO BE ADDED TO THE APP
...
#     async with AsyncConnectionPool(conninfo=os.getenv("DATABASE_URL"), kwargs=connection_kwargs) as pool:
#         app.state.pool = pool
#         global checkpointer, llmapp
...
#         llmapp=workflow.compile(checkpointer=checkpointer)
#         yield
#         print("lifespan(): Stop")

router = APIRouter()
checkpointer = None # to be set in lifespan
llmapp = None # to be set in lifespan
...
async def chat(request: LLMRequest):
    ...
    input_message = [HumanMessage(request.query)]
    response = await llmapp.ainvoke({"messages": input_message}, config=config)
    return response["messages"][-1].content

@router.get("/chat")
async def get_chat_history(id: str):
    config = {"configurable": {"thread_id": id}}
    state_snapshot = await llmapp.aget_state(config)
    messages = state_snapshot[0]["messages"]
    return [{"type":msg.type,"message":msg.content} for msg in messages]
